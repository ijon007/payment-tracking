...
export function calculatePaymentPlan(
  ...
  if (template.type === "30/70") {
    ...
    const thirdInstallment =
      remainingAmount - firstInstallment - secondInstallment;
...
export function calculateClientStatus(
  ...
  const hasOverdue = client.payments.some(
    (payment) => !payment.paidDate && payment.dueDate < today
  );
        ...
        value = clients.reduce((sum, client) => {
          const paidByMonth = client.payments
            .filter((p) => {
              ...
              const paidDate = new Date(p.paidDate);
              return (
                paidDate <= new Date(date.getFullYear(), date.getMonth() + 1, 0)
              );
            })
